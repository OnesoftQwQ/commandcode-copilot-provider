...
import { CommandCodeModelItem } from "./types";
...
export abstract class CommonApi<TMessage, TRequestBody> {
    ...
    protected adjustReadFileParameters(toolName: string, parameters: Record<string, unknown>): Record<string, unknown> {
        ...
        const defaultLines = config.get<number>("commandcode.readFileLines", 0);
        if (defaultLines <= 0) {
            return parameters;
        }
    ...
    public static prepareHeaders(
        ...
        if (vscode.workspace.getConfiguration("commandcode").get<boolean>("zeroDataRetention", false)) {
            headers["x-cmd-zdr"] = "1";
        }
