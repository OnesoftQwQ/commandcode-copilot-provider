...
import type { ModelPreset, CommandCodeModelItem } from "./types";
...
export class CommandCodeChatModelProvider implements LanguageModelChatProvider {
    ...
    async provideLanguageModelChatResponse(
        ...
        try {
            ...
            let um: CommandCodeModelItem | undefined = { ...getCatalogModelConfig(model.id) };
            ...
            if (um) {
                if (um.supportsTemperature !== false) {
                    const tempPreset = config.get<string>("commandcode.modelPreset", "custom");
                    if (tempPreset !== "custom") {
                        ...
                        const userTopP = config.get<number | null>("commandcode.top_p", null);
                        if (userTopP !== null) {
                            um.top_p = userTopP;
                        } else {
            ...
            const baseUrl = um?.baseUrl || getCatalogProviderBaseUrl("commandcode", "https://api.commandcode.ai/provider/v1/");
            ...
            const enableThirdPartyIndicator = config.get<boolean>("commandcode.enableThirdPartyTokenIndicator", true);
            ...
            const globalDelay = config.get<number>("commandcode.delay", 0);
            ...
            requestTimeoutMs = config.get<number>("commandcode.requestTimeout", 600000);
            ...
            if (isTimeout || isForceTerminated) {
                ...
                throw new Error(l10n("Request timed out. The generation took too long. You can increase the timeout in settings (commandcode.requestTimeout)."));
            }
    ...
    private async _handleInterceptedToolCall(params: {
        ...
        apiMode: string;
        model: LanguageModelChatInformation;
        um: CommandCodeModelItem | undefined;
        modelApiKey: string;
        baseUrl: string;
        ...
        const visionModelId = await resolveVisionProxyModelId(
            config.get<string>("commandcode.visionProxyModel", "Qwen/Qwen3.7-Plus")
        );
        const maxRounds = config.get<number>("commandcode.visionMaxRounds", 5);
        ...
        for (let round = 1; round <= maxRounds; round++) {
            ...
            const roundTimeoutMs = vscode.workspace.getConfiguration().get<number>("commandcode.requestTimeout", 600000);
