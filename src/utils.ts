...
import type { CommandCodeModelItem, RetryConfig } from "./types";
...
export function normalizeUserModels(models: unknown): CommandCodeModelItem[] {
    const list = Array.isArray(models) ? models : [];
    const out: CommandCodeModelItem[] = [];
    for (const item of list) {
        if (!item || typeof item !== "object") {
            continue;
        }
        const provider = getModelProviderId(item);
        out.push({ ...(item as CommandCodeModelItem), owned_by: provider });
    }
    return out;
}
